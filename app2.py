...
def clean_data(df):
    ...
    # Column name mapping from Hebrew to English - Updated for actual file structure
    column_mapping = {
        'תאריך': 'Date',
        'מקט': 'SKU',
        'תיאור מוצר': 'Product',
    ...
    # Enhanced date handling - Updated for DD/MM/YYYY format
        ...
        def convert_date(date_val):
            ...
            if isinstance(date_val, str):
                try:
                    ...
                    converted = pd.to_datetime(date_val, format='%d/%m/%Y', errors='coerce')
                    if pd.isna(converted):
                        # Try alternative formats
                        converted = pd.to_datetime(date_val, dayfirst=True, errors='coerce')
                    
                    if not pd.isna(converted):
                        current_year = datetime.now().year
                        if 2020 <= converted.year <= current_year + 5:
                            return converted
                    return pd.NaT
                except:
                    return pd.NaT
    ...
    # Category standardization - Updated for actual categories
    ...
    # Clean numeric columns - Updated for actual columns
    numeric_columns = ['UnitsSold', 'Stock', 'InventoryTurnover', 'DayOfWeek', 'Month', 'WeekOfYear']
    for col in numeric_columns:
        if col in df_clean.columns:
            ...
            # Handle extreme outliers for sales and stock
...
def prepare_forecast_data_enhanced(df):
    ...
    # Product features - Updated for actual data
    if 'Product' in df_clean.columns and 'Category' in df_clean.columns:
    ...
    # Use existing time features if available, otherwise derive from date
    if 'DayOfWeek' not in df_forecast.columns and 'Date' in df_forecast.columns:
        df_forecast['DayOfWeek'] = df_forecast['Date'].dt.dayofweek
    if 'Month' not in df_forecast.columns and 'Date' in df_forecast.columns:
        df_forecast['Month'] = df_forecast['Date'].dt.month
    if 'WeekOfYear' not in df_forecast.columns and 'Date' in df_forecast.columns:
        df_forecast['WeekOfYear'] = df_forecast['Date'].dt.isocalendar().week
    
    # Fill missing values in existing time features
    if 'DayOfWeek' in df_forecast.columns:
        df_forecast['DayOfWeek'] = df_forecast['DayOfWeek'].fillna(df_forecast['Date'].dt.dayofweek)
    if 'Month' in df_forecast.columns:
        df_forecast['Month'] = df_forecast['Month'].fillna(df_forecast['Date'].dt.month)
    if 'WeekOfYear' in df_forecast.columns:
        df_forecast['WeekOfYear'] = df_forecast['WeekOfYear'].fillna(df_forecast['Date'].dt.isocalendar().week)
    
    # Use inventory turnover if available
    if 'InventoryTurnover' in df_forecast.columns:
        df_forecast['InventoryTurnover'] = pd.to_numeric(df_forecast['InventoryTurnover'], errors='coerce').fillna(14)
    
    # Remove price and weight features since they're not in the data
...
def classify_products_by_cv(df):
    ...
    # Calculate statistics per SKU/Product
    ...
    st.write(f"Products with sufficient data: {len(product_stats)} out of {df['Product'].nunique()}")
    
    # CV threshold: 0.5
    cv_threshold = 0.5
    st.write(f"CV threshold: {cv_threshold}")
    ...
    for group, count in demand_groups.items():
        st.write(f"{group} demand: {count} products ({count/len(product_stats)*100:.1f}%)")
    
    # Statistics by group
    st.write("Statistics by demand group:")
